...
class TextureWrapping(texture: Texture) {
    var wrapS by property(TextureWrap.Repeat) { value ->
        glTextureParameteri(texture.id, GL_TEXTURE_WRAP_S, value.gl)
    }

    var wrapT by property(TextureWrap.Repeat) { value ->
        glTextureParameteri(texture.id, GL_TEXTURE_WRAP_T, value.gl)
    }

    var wrapR by property(TextureWrap.Repeat) { value ->
        glTextureParameteri(texture.id, GL_TEXTURE_WRAP_R, value.gl)
    }
